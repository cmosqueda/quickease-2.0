...
// --- SUB-COMPONENT: Flashcard Editor ---
const FlashcardSection = ({
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-4xl mx-auto">
      {/* 1. Preview Section (From LearnerAIFlashcardPage) */}
      <div className="flex flex-col items-center gap-4 bg-base-200 p-6 rounded-3xl">
        <h2 className="text-xl font-bold self-start flex gap-2 items-center">
          <Sparkles size={16} className="text-primary" /> Preview Deck
        </h2>
        <FlippableCard
      ...
      {/* 2. Edit List Section (From LearnerEditAIFlashcardPage) */}
...
export default function LearnerAICreateNotePage() {
  const data = useLoaderData() as any; // Loaded from localStorage
  const { user } = useAuth();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Attach Flashcards if they exist
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-base-100">
      ...
      <div className="sticky top-0 z-20 flex flex-col lg:flex-row justify-between lg:gap-0 gap-4 lg:items-center border-b border-base-300 p-4 bg-base-100/80 backdrop-blur-md">
        ...
        {/* TAB SWITCHER (Visible if resources exist) */}
        {(flashcards.length > 0 || questions.length > 0) && (
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-[1fr_0.2fr] h-full flex-1">
        ...
        <div className="flex flex-col gap-4 bg-base-100 border-l border-base-300 p-4 h-full sticky top-[80px]">
          ...
          <div className="divider text-xs opacity-50">Generate More</div>

          <button
            className="rounded-3xl btn btn-soft gap-2 join-item border border-base-300 shadow text-left justify-start"
            onClick={() => {
